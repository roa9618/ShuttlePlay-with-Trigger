...
public class GroupSessionAttendance extends BaseEntity {
    ...
    public static GroupSessionAttendance forMember(GroupSession session, GroupMember member) {
        ...
        entry.session = session; entry.member = member; entry.status = SessionAttendanceStatus.REGISTERED;
        return entry;
    }

    public static GroupSessionAttendance forGuest(GroupSession session, GroupSessionGuest guest) {
        GroupSessionAttendance entry = new GroupSessionAttendance();
        entry.session = session; entry.guest = guest; entry.status = SessionAttendanceStatus.REGISTERED;
        return entry;
    }

    public void arrive() {
        status = SessionAttendanceStatus.ARRIVED; arrivedAt = LocalDateTime.now(); expectedArrivalAt = null; lateReason = null;
    }

    public void late(LocalDateTime expectedArrivalAt, String reason) {
        status = SessionAttendanceStatus.LATE; this.expectedArrivalAt = expectedArrivalAt; lateReason = reason; arrivedAt = null;
    }

    public void absent() {
        status = SessionAttendanceStatus.ABSENT; expectedArrivalAt = null; lateReason = null; arrivedAt = null;
    }
}
