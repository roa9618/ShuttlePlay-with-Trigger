...
- 문서의 팀 평균/팀 내부 MMR 차이, 경기 수 차이, 연속 경기·휴식, 파트너·상대 중복 구간표를 정책 코드에서 그대로 사용합니다.
- 즐겜은 참여 기회와 중복 최소화, 빡겜은 팀 전력 균형 가중치를 강화합니다.
- `ANY`가 팀별 남녀 1명인 명확한 혼복 조합이면 혼복 MMR을 사용하고, 그 외에는 복식 MMR을 사용합니다.
- 우선 포함과 이번 매칭 제외 참가자를 운영 화면에서 선택할 수 있고, 유지할 후보만 남긴 재생성을 지원합니다.
- 3연속 경기 참가자는 일반 후보에서 제외하고 가능한 후보가 전혀 없을 때만 강한 벌점으로 유연 후보를 계산합니다.
- 회원과 게스트 모두 일정 참가 시점의 MMR 스냅샷과 일정 내 현재 MMR을 사용합니다. 회원 결과는 사용자 영구 MMR에도 동기화합니다.
- 결과 수정은 수정 경기 이후 같은 MMR 유형으로 연결된 경기들을 되돌린 뒤 시간순으로 다시 계산합니다.
- MMR 이력에는 K 값, 예상 승률, 점수차·신뢰도·팀 격차·책임도 계수, 하한·소프트캡 적용 여부와 변경 사유를 저장합니다.
- 결과 반영 후 일자별 경기 수, 승패, 득실점, 복식·혼복 MMR 변화 집계를 다시 저장합니다.
- 후보 생성은 완료 경기 수뿐 아니라 현재 경기와 대기 큐의 예정 횟수까지 합산하여 가장 적게 배정된 참가자를 우선합니다. 한 번에 여러 후보를 만들 때 참가자를 중복 사용하지 않습니다.
- 직전 경기를 뛴 참가자는 우선 후보에서 제외하여 `1경기 후 1회 휴식`을 적용합니다. 해당 조건으로 4명을 구성할 수 없는 경우에만 운영 지속을 위해 연속 경기 후보를 허용하고 기존 연속 경기 벌점을 함께 적용합니다.
- 일정 경기 기록 초기화 시 완료 경기를 역순으로 되돌려 영구 MMR을 복원한 뒤, 해당 일정의 경기·MMR 이력을 제거하고 같은 날짜의 다른 일정 경기가 있다면 그 기록만으로 일일 집계를 다시 계산합니다.
- 취소 경기는 경기 수, 휴식/연속 경기 판단, 파트너·상대 중복 이력, 운동 시간, MMR 및 리포트 집계에서 제외합니다.
