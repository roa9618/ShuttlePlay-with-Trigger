...
운영 화면에서 수동으로 만든 경기도 후보 큐를 거쳐 호출되므로 `match_queue_id`를 가집니다. 과거 데이터나 외부에서 가져온 경기만 `match_queue_id`가 없을 수 있습니다.
...
- `GroupSessionAttendance`를 회원과 게스트가 공통으로 사용하는 일정 참가자 식별자로 사용합니다.
- `SessionMatchQueue`와 `SessionMatchQueuePlayer`가 자동 매칭 후보와 팀 구성을 저장합니다.
- `SessionParticipantRelation`이 일정 내 고정 파트너, 같은 팀 선호·회피, 같은 경기 회피, 강한 조합 제한을 저장합니다.
- 실제 코트 경기는 기존 `MatchRecord`와 `MatchPlayer`를 확장해 참가자 화면, 개인 기록, 운영자 화면이 동일한 경기 ID를 사용합니다.
- `MatchResultRevision`은 결과 수정 전후 점수와 수정 사유를 보존합니다.
- 참가자 결과 입력과 운영자 결과 입력은 같은 결과 확정 서비스를 사용해 중복 반영을 방지합니다.
...
- `MatchingScorePolicy`가 자동 매칭 구간별 가산점·벌점 경계값을 관리합니다.
- `MmrCalculationPolicy`가 K 값, 예상 승률, 점수차·신뢰도·팀 격차·책임도·급수 보호 계산을 관리합니다.
- `DailyRecord`가 회원별 일자 집계를 저장하고 결과 입력·수정 시 다시 계산됩니다.
- `GroupSessionAttendance`가 회원/게스트 공통 MMR 스냅샷과 일정 내 현재 MMR, 연속 경기·휴식, 운영 메모를 보관합니다.
- 결과 수정은 연결된 후속 경기의 MMR을 역순으로 되돌리고 시간순으로 다시 적용합니다.
