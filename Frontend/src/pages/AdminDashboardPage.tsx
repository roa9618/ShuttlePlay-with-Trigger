...
import { encodePublicId, groupPath } from '../utils/publicId';
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    {selectedItem && <div className={styles.overlay} onMouseDown={() => !savingItem && setSelectedItem(null)}><section className={styles.userModal} onMouseDown={event => event.stopPropagation()}><header className={styles.modalHeader}><div><span className={styles.userAvatar}><Settings2 /></span><div><h2>{String(selectedItem['모임명'] ?? selectedItem['일정명'] ?? selectedItem['제목'] ?? selectedItem['체크 항목'] ?? `${config.title} 상세`)}</h2><p>{config.description}</p></div></div><Button variant="ghost" size="icon" onClick={() => setSelectedItem(null)} aria-label="닫기"><X /></Button></header><div className={styles.modalBody}>
      ...
      <div className={styles.itemActions}>
        {config.id === 'groups' && <><Button variant="outline" className={styles.secondaryButton} onClick={() => navigate(groupPath(String(selectedItem.id)))}>모임 페이지 열기</Button><Button disabled={savingItem || selectedItem.상태 === 'ACTIVE'} onClick={() => void runItemAction('ACTIVE')}>활성 처리</Button><Button variant="destructive" disabled={savingItem || selectedItem.상태 === 'INACTIVE'} onClick={() => void runItemAction('INACTIVE')}>비활성 처리</Button></>}
        {config.id === 'sessions' && <><Button variant="outline" className={styles.secondaryButton} onClick={() => navigate(`${groupPath(String(selectedItem['모임 ID']), '/schedule')}?sessionId=${encodePublicId(String(selectedItem.id))}`)}>일정 페이지 열기</Button><Button variant="destructive" disabled={savingItem || selectedItem.상태 === 'CANCELLED' || selectedItem.상태 === 'CLOSED'} onClick={() => void runItemAction('cancel')}>일정 취소</Button></>}
