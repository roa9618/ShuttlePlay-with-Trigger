...
import { decodePublicId, groupPath, sessionPath } from '../utils/publicId';
...
type ScheduleItem = { id: number; entryCode?: string; title: string; startsAt: string; endsAt: string | null; date: string; time: string; place: string; courtCount: number; joined: number; undecided: number; absent: number; guests: number; deadline: string; status: ScheduleStatus; sessionType: string; voteStatus: VoteStatus; matches: number | null; votingAllowed: boolean; guestAllowed: boolean; guestLinkAllowed: boolean };
...
function toScheduleItem(session: GroupSessionResponse): ScheduleItem {
  ...
  return {
    ...
    voteStatus: normalizeVoteStatus(session.myVoteStatus),
    matches: null,
    votingAllowed: session.votingAllowed ?? true,
    guestAllowed: session.guestAllowed ?? true,
    guestLinkAllowed: session.guestLinkAllowed ?? false,
  };
}

function toPostItem(post: GroupPostResponse): PostItem {
  return {
    id: post.id,
...
function toMemberItem(member: GroupMemberResponse): MemberItem {
...
function toOperationItem(item: Record<string, unknown>): OperationItem {
...
export default function GroupDetailPage() {
  ...
  useEffect(() => {
    const sessionId = Number(decodePublicId(new URLSearchParams(location.search).get('sessionId') ?? undefined));
  ...
  useEffect(() => {
    const createSession = new URLSearchParams(location.search).get('createSession');
    if (activeTab === 'schedule' && createSession === 'true' && canManageSchedule && groupInfo) {
      setModal({ type: 'createSchedule' });
    }
  }, [activeTab, canManageSchedule, groupInfo, location.search]);
  ...
  useEffect(() => {
    ...
    if (!hasPermission) {
      navigate(groupPath(numericGroupId), { replace: true });
    }
  }, [activeTab, canManageGuests, canManageRequests, canViewHistory, isOwner, navigate, numericGroupId]);

  const selectedSchedule = modal?.type === 'schedule'
    ? scheduleItems.find(schedule => schedule.id === modal.id)
    : undefined;
  const upcomingSchedule = dashboard?.upcomingSession
    ? toScheduleItem(dashboard.upcomingSession)
    : scheduleItems.find(schedule => schedule.status !== 'COMPLETED' && schedule.status !== 'CANCELLED');
  ...
  const handleCopyGroupLink = async () => {
    await navigator.clipboard?.writeText(`${window.location.origin}${groupPath(numericGroupId, '/join')}`);
    showToast('모임 공유 링크를 복사했습니다.');
  };

  const handleCopySessionGuestLink = async (sessionId: number) => {
    const schedule = scheduleItems.find(item => item.id === sessionId);
    const code = schedule?.entryCode ?? (dashboard?.upcomingSession?.id === sessionId ? dashboard.upcomingSession.entryCode : undefined);
    await navigator.clipboard?.writeText(`${window.location.origin}${sessionPath(sessionId, '/join')}${code ? `?code=${encodeURIComponent(code)}` : ''}`);
    showToast('운동 일정 공유 링크를 복사했습니다.');
  };

  const handleCloseModal = () => {
    setModal(null);
    const searchParams = new URLSearchParams(location.search);
    if (searchParams.has('postId') || searchParams.has('sessionId') || searchParams.has('createSession')) {
  ...
  const handleVote = (nextStatus: VoteStatus, targetSessionId?: number) => {
    ...
    if (sessionId) {
      void groupDetailApi.vote(numericGroupId, sessionId, nextStatus === 'JOIN' ? 'ATTENDING' : nextStatus)
        .then(() => {
          setScheduleItems(current => current.map(schedule => schedule.id === sessionId ? { ...schedule, voteStatus: nextStatus } : schedule));
          setDashboard(current => current?.upcomingSession?.id === sessionId
            ? { ...current, upcomingSession: { ...current.upcomingSession, myVoteStatus: nextStatus === 'JOIN' ? 'ATTENDING' : nextStatus } }
            : current);
  ...
  const handleEnterSchedule = (schedule: ScheduleItem) => {
    navigate(sessionPath(schedule.id, '/join'));
  };
    ...
    <div className = {styles.page}>
      ...
      <div className = {styles.pageShell}>
        ...
        <nav className = {styles.tabBar} style = {{ gridTemplateColumns: `repeat(${visibleTabs.length}, minmax(0, 1fr))` }}>
          {visibleTabs.map(tab => (
            <Link key = {tab.key} to = {groupPath(numericGroupId, tabPaths[tab.key] ? `/${tabPaths[tab.key]}` : '')} className = {styles.tabButton(activeTab === tab.key)}>
              {tab.label}
        ...
        <main className = {styles.tabContent}>
          {activeTab === 'home' && (
            <HomeTab
              ...
              onOperateSchedule = {schedule => navigate(sessionPath(schedule.id, '/dashboard'))}
              onCreateSchedule = {() => setModal({ type: 'createSchedule' })}
              onNavigateReport = {id => navigate(sessionPath(id, '/report'))}
            />
          )}
          ...
          {activeTab === 'schedule' && (
            <ScheduleTab
              ...
              onOpenSchedule = {handleScheduleTabSelection}
              onCreateSchedule = {() => setModal({ type: 'createSchedule', initialDate: `${calendarYear}-${String(calendarMonth).padStart(2, '0')}-${String(selectedDay).padStart(2, '0')}` })}
            />
          )}
      ...
      {modal && (
        <Modal title = {getModalTitle(modal, selectedSchedule?.title, selectedPost?.title, selectedMember?.name, selectedGuest?.name)} onClose = {handleCloseModal} onBack = {modal.type === 'editGuest' ? () => setModal({ type: 'participants', id: modal.id }) : modal.type === 'participants' || modal.type === 'addGuest' || modal.type === 'manageSchedule' ? () => setModal({ type: 'schedule', id: modal.id }) : modal.type === 'memberPermissions' || modal.type === 'ownershipTransfer' || modal.type === 'memberRemoval' ? () => setModal({ type: 'member', id: modal.id }) : undefined} centeredHeader = {modal.type === 'createSchedule' || modal.type === 'schedule' || modal.type === 'post' || modal.type === 'writePost' || modal.type === 'member' || modal.type === 'guest' || modal.type === 'groupDeletion'} fixedScheduleSize = {modal.type === 'createSchedule' || modal.type === 'schedule' || modal.type === 'post' || modal.type === 'participants' || modal.type === 'addGuest' || modal.type === 'editGuest' || modal.type === 'manageSchedule' || modal.type === 'writePost' || modal.type === 'member' || modal.type === 'guest' || modal.type === 'memberPermissions' || modal.type === 'ownershipTransfer' || modal.type === 'memberRemoval' || modal.type === 'groupDeletion'}>
          ...
          {selectedSchedule && (
            <ScheduleModal schedule = {selectedSchedule} canManage = {canManageSchedule} canOperate = {canManageSchedule && canOperateSchedule(selectedSchedule)} canAddGuest = {canManageSchedule && selectedSchedule.guestAllowed} canChangeVote = {canChangeVoteForSchedule(selectedSchedule, sameDayVoteChangeAllowed, postDeadlineVoteChangeAllowed)} canEnter = {canEnterSchedule(selectedSchedule, currentTime) && !(selectedSchedule.voteStatus === 'ABSENT' && !canChangeVoteForSchedule(selectedSchedule, sameDayVoteChangeAllowed, postDeadlineVoteChangeAllowed))} onVote = {status => handleVote(status, selectedSchedule.id)} onEnter = {() => handleEnterSchedule(selectedSchedule)} onOperate = {() => navigate(sessionPath(selectedSchedule.id, '/dashboard'))} onOpenParticipants = {id => setModal({ type: 'participants', id })} onAddGuest = {id => setModal({ type: 'addGuest', id })} onManage = {id => setModal({ type: 'manageSchedule', id })} onShareGuestLink = {handleCopySessionGuestLink} />
          )}
...
function HomeTab({
  ...
  currentTime, onCancelNotice, onSaveNotice, onDeleteNotice, onVote, onOpenSchedule, onEnterSchedule, onOperateSchedule,
  ...
  onOperateSchedule: (schedule: ScheduleItem) => void;
  ...
  const canEnter = canEnterSchedule(upcoming, currentTime) && !(upcoming?.voteStatus === 'ABSENT' && !canChangeVote);
              ...
              <div className = {styles.upcomingMain}>
                ...
                <div className = {styles.upcomingText}>
                  <div className = {styles.inlineBadges}>
                    ...
                    <Badge variant = "outline">{getVoteLabel(upcoming.voteStatus)}</Badge>
                  </div>
...
function ScheduleTab({ scheduleItems, year, month, selectedDay, daySchedules, monthlySummary, canManage, onSelectDay, onChangeMonth, onOpenSchedule, onCreateSchedule }: {
  ...
  const getDisplayStatus = (schedule: ScheduleItem): ScheduleStatus => schedule.date < todayDate ? 'COMPLETED' : schedule.status;
    ...
    <div className = {styles.paginatedTab}>
      <Panel title = "게스트 관리" description = "종료된 운동에 실제 참여한 게스트를 확인하고 메모를 관리하세요." icon = {UserCheck} className = {styles.listTabPanel} bodyClassName = {styles.listTabBody}>
        ...
        {filteredGuests.length > 0 ? (
          ...
          <EmptyState icon = {UserCheck} title = "조건에 맞는 게스트가 없습니다." description = "종료된 운동에 참여한 게스트가 있으면 이곳에서 확인할 수 있습니다." />
        )}
...
function HistoryTab({ items, currentPage, totalPages, onPageChange }: { items: OperationItem[]; currentPage: number; totalPages: number; onPageChange: (page: number) => void }) {
  return (
    <div className = {styles.paginatedTab}>
...
function ScheduleModal({ schedule, canManage, canOperate, canAddGuest, canChangeVote, canEnter, onVote, onEnter, onOperate, onOpenParticipants, onAddGuest, onManage, onShareGuestLink }: { schedule: ScheduleItem; canManage: boolean; canOperate: boolean; canAddGuest: boolean; canChangeVote: boolean; canEnter: boolean; onVote: (status: VoteStatus) => void; onEnter: () => void; onOperate: () => void; onOpenParticipants: (id: number) => void; onAddGuest: (id: number) => void; onManage: (id: number) => void; onShareGuestLink: (id: number) => void }) {
  return (
    <div className = {styles.scheduleModalContent}>
      ...
      {(canEnter || canManage || canOperate) && <div className = {styles.modalActions}>{canAddGuest && <Button variant = "outline" className = {styles.roundButton} onClick = {() => onAddGuest(schedule.id)}><Plus /> 게스트 추가</Button>}{canManage && <Button variant = "outline" className = {styles.roundButton} onClick = {() => onManage(schedule.id)}><Settings /> 일정 관리</Button>}{canOperate && <Button className = {styles.roundButton} onClick = {onOperate}><Activity /> 경기 운영</Button>}{canEnter && <Button className = {styles.roundButton} onClick = {onEnter}>입장</Button>}</div>}
    </div>
  );
}
...
function MemberModal({ groupId, member, isOwner, canViewMemo, canManage, onOpenPermissions, onOpenOwnershipTransfer, onOpenRemoval }: { groupId: number; member: MemberItem; isOwner: boolean; canViewMemo: boolean; canManage: boolean; onOpenPermissions: () => void; onOpenOwnershipTransfer: () => void; onOpenRemoval: () => void }) {
...
function OwnershipTransferModal({ groupId, member, onComplete }: { groupId: number; member: MemberItem; onComplete: (message: string) => void }) {
...
function MemberRemovalModal({ groupId, member, onComplete }: { groupId: number; member: MemberItem; onComplete: (message: string) => void }) {
  const [confirmation, setConfirmation] = useState('');
  return (
    <div className = {styles.memberManagementModal}>
  ...
  return (
    <div className = {styles.participantsModalContent}>
      ...
      <div className = {styles.participantList}>
        {filteredParticipants.map(participant => <div key = {`${participant.guest ? 'guest' : 'member'}-${participant.id}`} className = {styles.participantCard}>{participant.profileImageUrl ? <img src = {participant.profileImageUrl} alt = {`${participant.name} 프로필`} className = {styles.participantProfileImage} onError = {event => { event.currentTarget.style.display = 'none'; event.currentTarget.nextElementSibling?.removeAttribute('hidden'); }} /> : null}<div hidden = {!!participant.profileImageUrl} className = {participant.guest ? styles.guestAvatar : styles.participantAvatar}>{participant.name[0]}</div><span><strong>{participant.name}</strong><small>{formatGender(participant.gender)} · {formatAgeGroup(participant.ageGroup)} · {formatGrade(participant.grade)}</small></span><div className = {styles.participantBadges}><Badge variant = "outline" className = {participant.guest ? styles.guestBadge : styles.participantRoleBadge}>{participant.guest ? '게스트' : participant.role === 'OWNER' ? '소유자' : participant.role === 'MANAGER' ? '매니저' : '멤버'}</Badge><Badge className = {styles.statusBadge(filter)}>{getVoteLabel(filter)}</Badge>{participant.guest && canManage && <Button variant = "ghost" size = "sm" className = {styles.participantManageButton} onClick = {() => onEditGuest(participant)}>관리</Button>}</div></div>)}
      </div>
    </div>
  );
}

function CreateScheduleModal({ groupId, groupGuestAllowed, initialDate, onComplete, onError }: { groupId: number; groupGuestAllowed: boolean; initialDate?: string; onComplete: () => Promise<void>; onError: (error: unknown) => void }) {
  const today = new Date();
  ...
  const [guestAllowed, setGuestAllowed] = useState(groupGuestAllowed);
  const [guestLinkAllowed, setGuestLinkAllowed] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const startsAt = date && startTime ? `${date}T${startTime}` : '';
  const endsAt = date && endTime ? `${date}T${endTime}` : '';
  const validPeriod = Boolean(startsAt && endsAt && endsAt > startsAt);
  const validDeadline = !votingAllowed || Boolean(voteDeadline && voteDeadline < startsAt);
  const canSubmit = Boolean(title.trim() && date && startTime && endTime && place.trim() && Number(courtCount) >= 1 && validPeriod && validDeadline && !submitting);

  const submit = async () => {
    if (!canSubmit) return;
    setSubmitting(true);
    try {
      await groupDetailApi.createSession(groupId, {
        title: title.trim(),
        startsAt,
        endsAt,
        place: place.trim(),
        ...
        votingAllowed,
        voteDeadline: votingAllowed ? voteDeadline : null,
        guestAllowed,
        guestLinkAllowed,
      });
      await onComplete();
  ...
  return (
    <div className = {styles.createScheduleModalContent}>
      <div className = {styles.createScheduleGrid}>
        <label className = {styles.modalField}><span>일정명</span><Input value = {title} onChange = {event => setTitle(event.target.value)} className = {styles.input} placeholder = "일정명을 입력하세요" /></label>
        <label className = {styles.modalField}><span>일정 구분</span><Select value = {sessionType} onValueChange = {setSessionType}><SelectTrigger className = {styles.selectTriggerWide}><SelectValue /></SelectTrigger><SelectContent>{[['REGULAR', '정기 모임'], ['LIGHTNING', '번개 모임'], ['EXCHANGE', '교류전'], ['TOURNAMENT', '대회'], ['OTHER', '기타']].map(([value, label]) => <SelectItem className = {styles.guestSelectItem} key = {value} value = {value}>{label}</SelectItem>)}</SelectContent></Select></label>
      </div>
      <div className = {styles.createScheduleTimeGrid}>
        <label className = {styles.modalField}><span>날짜</span><Input type = "date" min = {localToday} value = {date} onChange = {event => setDate(event.target.value)} className = {styles.input} /></label>
        <label className = {styles.modalField}><span>시작 시간</span><Input type = "time" value = {startTime} onChange = {event => setStartTime(event.target.value)} className = {styles.input} /></label>
        <label className = {styles.modalField}><span>종료 시간</span><Input type = "time" value = {endTime} onChange = {event => setEndTime(event.target.value)} className = {styles.input} /></label>
      </div>
      {!validPeriod && date && startTime && endTime && <p className = {styles.createScheduleValidation}>종료 시간은 시작 시간보다 늦어야 합니다.</p>}
      <div className = {styles.createScheduleGrid}>
        <label className = {styles.modalField}><span>장소</span><Input value = {place} onChange = {event => setPlace(event.target.value)} className = {styles.input} placeholder = "운동 장소를 입력하세요" /></label>
        <label className = {styles.modalField}><span>코트 수</span><Input type = "number" min = "1" value = {courtCount} onChange = {event => setCourtCount(event.target.value)} className = {styles.input} /></label>
      </div>
      <div className = {styles.createScheduleOptions}>
        <SettingSwitch title = "참여 투표 허용" description = "모임 멤버가 참여·미정·불참 상태를 선택할 수 있습니다." checked = {votingAllowed} onCheckedChange = {checked => { setVotingAllowed(checked); if (!checked) setVoteDeadline(''); }} />
        {votingAllowed && <label className = {styles.modalField}><span>참여 투표 마감 일시</span><Input type = "datetime-local" max = {startsAt || undefined} value = {voteDeadline} onChange = {event => setVoteDeadline(event.target.value)} className = {styles.input} /></label>}
        {votingAllowed && voteDeadline && !validDeadline && <p className = {styles.createScheduleValidation}>투표 마감 일시는 일정 시작 전이어야 합니다.</p>}
        <SettingSwitch title = "게스트 허용" description = {groupGuestAllowed ? "운영자가 이 일정에 게스트를 추가할 수 있습니다." : "모임 설정에서 게스트 일정 참여가 허용되어 있지 않습니다."} checked = {guestAllowed} disabled = {!groupGuestAllowed} onCheckedChange = {checked => { setGuestAllowed(checked); if (!checked) setGuestLinkAllowed(false); }} />
        <SettingSwitch title = "비회원 링크 참여" description = "게스트가 전용 링크를 통해 이 일정에 참여할 수 있습니다." checked = {guestLinkAllowed} disabled = {!guestAllowed} onCheckedChange = {setGuestLinkAllowed} />
      </div>
      <div className = {styles.modalActions}><Button disabled = {!canSubmit} className = {styles.roundButton} onClick = {() => void submit()}><Plus /> {submitting ? '일정 생성 중' : '운동 일정 만들기'}</Button></div>
    </div>
  );
}
...
function ManageScheduleModal({ groupId, schedule, onComplete }: { groupId: number; schedule?: ScheduleItem; onComplete: (message: string) => void }) {
  ...
  const [courtCount, setCourtCount] = useState(String(schedule?.courtCount ?? 1));
...
function normalizeVoteStatus(status?: string | null): VoteStatus {
  if (status === 'ATTENDING' || status === 'JOIN') return 'JOIN';
...
function canChangeVoteForSchedule(schedule: ScheduleItem | undefined, sameDayAllowed: boolean, postDeadlineAllowed: boolean) {
  ...
  if (!schedule.votingAllowed) return false;
...
function canOperateSchedule(schedule: ScheduleItem | undefined) {
  return !!schedule && schedule.status !== 'COMPLETED' && schedule.status !== 'CANCELLED';
}
