...
export default function MatchQueuePage() {
  ...
  const filteredParticipants = sortedParticipants.filter(item => item.name.includes(planParticipantSearch.trim()));
  const manualFilteredParticipants = sortedParticipants.filter(item => item.name.includes(manualSearch.trim()));
  const generatedMessage = (message: string, count: number) => `${message} 총 ${count}경기가 추가됐어요.`;
  ...
  const generate = async () => {
    ...
    setNotice(generatedMessage(result.message, result.generatedCount)); await load();
  };
  const generatePlan = async () => {
    try {
      ...
      setNotice(generatedMessage(result.message, result.generatedCount)); await load();
    } catch { setNotice('모든 참가자의 보장 경기 수를 채울 수 없어요. 경기 가능한 참가자와 제외 설정을 확인해 주세요.'); }
  };
  const createManual = async (keepOpen = false) => {
    const ids = manualPlayers.map(Number);
    if (ids.some(id => !id) || new Set(ids).size !== 4) { setNotice('서로 다른 참가자 4명을 선택해 주세요.'); return; }
    try {
      await sessionOperationApi.createManualQueue(sessionId, { teamAIds: ids.slice(0, 2), teamBIds: ids.slice(2), matchType, playStyle });
      setManualOpen(keepOpen); setManualPlayers(['', '', '', '']); setNotice(keepOpen ? '1경기가 추가됐어요. 다음 경기를 바로 선택해 주세요.' : '직접 선택한 1경기를 후보 큐에 추가했어요.'); await load();
    } catch { setNotice('선택한 참가자 상태 또는 현재 경기 유형을 확인해 주세요.'); }
  };
  ...
  const cloneQueue = (queue: OperationQueue) => {
    ...
    setMatchType(queue.matchType); setPlayStyle(queue.playStyle ?? 'FUN'); setManualOpen(true);
  };
  const assignEmptyCourts = async () => {
    ...
    catch { setNotice('빈 코트에 호출 가능한 경기 후보를 다시 확인해 주세요.'); }
  };
  const updateAutoFill = async () => {
    const enabled = !data?.autoCourtAssignmentEnabled;
    try { const result = await sessionOperationApi.updateAutoFill(sessionId, enabled); setNotice(result.message); await load(); }
    catch { setNotice('자동 채우기 상태를 변경하지 못했어요. 경기 배정 시작 후 다시 시도해 주세요.'); }
  };
  ...
  const saveEdit = async () => {
    if (!editing || new Set(selectedPlayers).size !== 4) { setNotice('서로 다른 참가자 4명을 선택해 주세요.'); return; }
  ...
  return <SessionOperationShell title="경기 후보 큐" description="대진표를 준비한 뒤 게임 배정을 시작하면, 이후 빈 코트는 자동 채우기 설정에 따라 다음 경기로 채워져요." actions={<>{data?.matchAssignmentStarted ? <Button variant={data.autoCourtAssignmentEnabled ? 'default' : 'outline'} className={data.autoCourtAssignmentEnabled ? `${s.primaryButton} hover:bg-primary/90 hover:text-primary-foreground` : 'h-12 rounded-xl border-border px-5 text-base font-bold hover:border-border hover:bg-secondary hover:text-foreground'} onClick={() => void updateAutoFill()}><Play className={s.iconSm} />{data.autoCourtAssignmentEnabled ? '자동 채우기 ON' : '자동 채우기 OFF'}</Button> : <Button className={s.primaryButton} onClick={() => void assignEmptyCourts()}><Play className={s.iconSm} />게임 배정 시작</Button>}<Button variant="outline" className={s.outlineButton} onClick={() => void load()}><RefreshCw className={s.iconSm} />새로고침</Button></>}>
    {!data ? <div className={s.empty}>경기 후보를 불러오고 있어요.</div> : <div className={s.stack}>
      <section className={s.card}>
        <div className="grid grid-cols-4 gap-4">
          ...
          <label><span className={s.label}>1인 보장 경기 수</span><input type="number" inputMode="numeric" min={1} max={20} value={targetGames} onChange={event => setTargetGames(Math.max(1, Math.min(20, Number(event.target.value) || 1)))} className="box-border block h-12 w-full rounded-xl border border-border bg-background px-3 py-0 text-base font-bold leading-none transition-[color,box-shadow] hover:border-primary/60 focus:border-primary/60 focus:outline-none focus:ring-[3px] focus:ring-primary/20 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" /></label>
        ...
        <div className="mt-5 rounded-2xl border border-border bg-muted/20 p-4">
          <div className="flex items-center justify-between gap-4"><div><strong className="text-base">참가자별 예외 설정</strong><p className="mt-1 text-sm text-muted-foreground">필요한 사람만 선택하세요. 선택하지 않은 모든 경기 가능 참가자는 보장 경기 수만큼 배정합니다.</p></div><input value={planParticipantSearch} onChange={event => setPlanParticipantSearch(event.target.value)} className="h-10 w-64 rounded-xl border border-border bg-background px-3 text-sm font-semibold focus:border-primary focus:outline-none" placeholder="참가자 이름 검색" /></div>
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="rounded-xl border border-primary/20 bg-background p-4"><div className="flex items-start justify-between gap-3"><div><strong>우선 배정</strong><p className="mt-1 text-sm text-muted-foreground">0경기 참가자처럼 먼저 챙겨야 하는 사람을 동점 조합에서 우선 고려합니다.</p></div>{forcedPlayers.length > 0 && <button type="button" className="text-sm font-bold text-primary" onClick={() => setForcedPlayers([])}>전체 해제</button>}</div><div className="mt-3 grid max-h-40 grid-cols-4 gap-2 overflow-y-auto pr-1">{filteredParticipants.map(item => <button type="button" key={item.attendanceId} onClick={() => toggleForcedPlayer(item.attendanceId)} className={`min-h-12 w-full rounded-xl border px-2 py-2 text-center text-sm font-bold ${forcedPlayers.includes(item.attendanceId) ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:border-primary'}`}><span className="block truncate">{item.name}</span><small className="block opacity-70">{item.games}경기</small></button>)}</div></div>
            <div className="rounded-xl border border-destructive/20 bg-background p-4"><div className="flex items-start justify-between gap-3"><div><strong>이번 생성에서 제외</strong><p className="mt-1 text-sm text-muted-foreground">참가자 상태와 별개로, 운영자가 특별히 이번 생성에서만 빼야 하는 사람을 선택합니다.</p></div>{excludedPlayers.length > 0 && <button type="button" className="text-sm font-bold text-destructive" onClick={() => setExcludedPlayers([])}>전체 해제</button>}</div><div className="mt-3 grid max-h-40 grid-cols-4 gap-2 overflow-y-auto pr-1">{filteredParticipants.map(item => <button type="button" key={item.attendanceId} onClick={() => toggleExcludedPlayer(item.attendanceId)} className={`min-h-12 w-full rounded-xl border px-2 py-2 text-center text-sm font-bold ${excludedPlayers.includes(item.attendanceId) ? 'border-destructive bg-destructive text-destructive-foreground' : 'border-border bg-background hover:border-destructive'}`}><span className="block truncate">{item.name}</span><small className="block truncate opacity-70">{genderLabels[item.gender] ?? item.gender} · {ageLabels[item.ageGroup] ?? item.ageGroup} · {item.grade}급</small></button>)}</div></div>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-2xl border border-border p-4"><div><strong>지금 필요한 후보만 추가</strong><p className="mt-1 text-sm text-muted-foreground">전체 대진표 대신 현재 경기 유형, 운영 방식, 예외 설정 기준으로 코트 수만큼 추천 경기를 큐 마지막에 추가합니다.</p></div><Button variant="outline" className={s.outlineButton} onClick={() => void generate()}><Sparkles className={s.iconSm} />코트 수만큼 후보 추가</Button><Button variant="outline" className={s.outlineButton} onClick={() => setManualOpen(true)}><ListPlus className={s.iconSm} />직접 경기 추가</Button></div>
      </section>
    ...
    {manualOpen && data && <div className={s.modalBackdrop}><div className={s.modal}><div className={s.between}><div><h2 className={s.modalTitle}>직접 경기 추가</h2><p className={s.sectionDescription}>참가자를 누르는 순서대로 A팀 2명, B팀 2명이 구성돼요. 상단의 경기 유형과 운영 방식으로 후보 큐에 추가됩니다.</p></div><button aria-label="닫기" onClick={() => setManualOpen(false)}><X /></button></div><div className="mt-5 grid grid-cols-4 gap-2">{manualPlayers.map((value, index) => <button type="button" key={index} onClick={() => value && selectManualPlayer(Number(value))} className="min-h-16 rounded-xl border border-primary/30 bg-primary/5 px-2 text-sm font-bold"><small className="block text-primary">{index < 2 ? `A팀 ${index + 1}` : `B팀 ${index - 1}`}</small>{value ? data.participants.find(item => item.attendanceId === Number(value))?.name : '선택'}</button>)}</div><input value={manualSearch} onChange={event => setManualSearch(event.target.value)} className="mt-4 h-11 w-full rounded-xl border border-border bg-background px-4 text-sm font-semibold focus:border-primary focus:outline-none" placeholder="이름으로 빠르게 찾기" /><div className="mt-3 grid max-h-52 grid-cols-3 gap-2 overflow-y-auto pr-1">{manualFilteredParticipants.map(item => <button type="button" key={item.attendanceId} onClick={() => selectManualPlayer(item.attendanceId)} className={`min-h-12 rounded-xl border px-3 text-left text-sm font-bold ${manualPlayers.includes(String(item.attendanceId)) ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:border-primary'}`}><span className="block">{item.name}</span><small className="opacity-70">{item.grade}급 · {item.games}경기{item.affiliation ? ` · ${item.affiliation}` : ''}</small></button>)}</div><div className="mt-6 grid grid-cols-2 gap-3"><Button variant="outline" className={s.outlineButton} onClick={() => void createManual(true)}>추가하고 계속</Button><Button className={s.primaryButton} onClick={() => void createManual(false)}>추가하고 닫기</Button></div></div></div>}
