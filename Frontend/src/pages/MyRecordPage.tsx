...
function Highlight({ title, value, match }: { title: string; value: string; match?: MatchRecordItem }) { const content = <><Flame /><span>{title}</span><strong>{value}</strong>{match && <small>{formatDate(match.playedAt)} · {match.groupName}</small>}</>; return match ? <Link to={`/sessions/${match.sessionId}/my-report`} className={styles.highlight}>{content}</Link> : <div className={styles.highlight}>{content}</div>; }
