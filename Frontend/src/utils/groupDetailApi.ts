...
export const groupDetailApi = {
  ...
  getJoinPreview: (id: number) => isGalleryPreviewMode() ? Promise.resolve({ groupId: id, groupName: previewGroup.name, profileImageUrl: previewGroup.profileImageUrl, approvalRequired: true, status: 'AVAILABLE' as const }) : apiClient.get<GroupJoinLinkResponse>(`${root(id)}/join`, auth),
