...
export function getNotifications(page = 0, size = 100, unreadOnly = false) {
  if (isGalleryPreviewMode()) {
    const items = [
      { id: 1, type: 'SCHEDULE', title: '일정 참여자가 도착했어요', message: '홍길동님이 도착 완료 상태로 변경했습니다.', targetPath: '/groups/1/schedule', read: false, createdAt: previewNow(0, 18, 55) },
      ...
      { id: 3, type: 'GROUP', title: '새 가입 요청이 있어요', message: '최셔틀님이 모임 가입을 요청했습니다.', targetPath: '/groups/1/requests', read: true, createdAt: previewNow(-1) },
